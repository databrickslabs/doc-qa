...
class ParameterDef:
    def __init__(self, name, type, display_name=None, description=None, regex_rule=None):
        self.name = name
        ...
        self.regex_rule = regex_rule or self.default_regex()  # use default if none provided
...
class BaseLlmEvaluator:
    def __init__(self, grading_prompt_tempate: PromptTemplate, input_columns: list, 
                 model: str, temperature: float, openai_function: dict = None, 
                 output_parameters: list = None, system_prompt_template: PromptTemplate = None, 
        ...
        if openai_function is None and output_parameters is None:
            raise ValueError("Either openai_function or output_parameters should be provided.")
        ...
        if openai_function is not None and output_parameters is not None:
            raise ValueError("Only one of openai_function and output_parameters should be provided.")
        ...
        for input_column in input_columns:
            if input_column not in self.grading_prompt_tempate.variables:
                raise ValueError(f"Input column '{input_column}' not found in grading prompt template variables.")
    ...
    def extract_parameters(self, function_json):
        ...
        for name, parameter in parameters.items():
            ...
            parameter_defs.append(ParameterDef(name=name, type=parameter_type, description=description))
    ...
    def run_eval(self, concurrency: int = 10, dataset_df: DataFrame = None, catch_error=True) -> EvalResult:
        rows = []
        with concurrent.futures.ThreadPoolExecutor(max_workers=concurrency) as executor:
            future_to_row = {executor.submit(self.grade_row_retry_wrapper, RowInput(**row)): row for index, row in dataset_df.iterrows()}
            for future in concurrent.futures.as_completed(future_to_row):
                ...
                expected_attributes = ["is_successful", "error_msg"] + [parameter.name for parameter in self.output_parameters] + self.input_columns
                original_row = future_to_row[future]
                try:
                    result = future.result()
                    # filter for the expected attributes
                    result_dict = {key: value for key, value in result.__dict__.items() if key in expected_attributes}
                    # add the original row to the result, only on the input columns
                    result_dict.update({key: value for key, value in original_row.items() if key in self.input_columns})
                    ...
                    if catch_error:
                        logger.warn(f"Encountered error while processing row, error: {exc}, original row {original_row}")
                        ...
                        row_eval_result = RowEvalResult(
                            is_successful=False,
                            error_msg=str(exc),
                            **{key: None for key in expected_attributes if key not in ["is_successful", "error_msg"]}
                        )                    
                        rows.append(row_eval_result)
                    else:
                        raise exc
        ...
        for parameter in self.output_parameters:
            if parameter.type == ParameterType.NUMBER:
                ...
                parameter_values = [row.__dict__[parameter.name] for row in rows if row.is_successful]
...
class OpenAIEvaluator(BaseLlmEvaluator):
    ...
    def __init__(self, model: str, temperature: float, grading_prompt_tempate: PromptTemplate, input_columns: list, 
        ...
        super().__init__(model=model, temperature=temperature, grading_prompt_tempate=grading_prompt_tempate, 
                         ...
                         system_prompt_template=system_prompt_template, retry_policy=retry_policy)
    ...
    def grade_row(self, row_input: RowInput) -> RowEvalResult:
        ...
        response_message = openai_provider.request_openai(messages=messages, functions=functions, model=self.model, temperature=self.temperature)
        if "function_call" in response_message:
            ...
            output_values = [arguments[output_parameter.name] for output_parameter in self.output_parameters]
            row_eval_result = RowEvalResult(
                is_successful=True,
                error_msg=None,
                **{output_parameter.name: output_value for output_parameter, output_value in zip(self.output_parameters, output_values)}
            )
            logger.debug(f"Successfully got result {row_eval_result} ")
            return row_eval_result
        else:
            logger.warning(f"Retrying, OpenAI doesn't call function for row {row_input}, response_message: {response_message}")
            return None
...
class AnthropicEvaluator(BaseLlmEvaluator):
    ...
    def grade_row(self, row_input: RowInput) -> RowEvalResult:
        ...
        if None in extracted_data.values():
            attributes_missing_values = [key for key, value in extracted_data.items() if value is None]
            logger.warning(f"We are missing value for some output parameters {attributes_missing_values}")
            return None
        else:
