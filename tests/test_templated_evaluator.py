...
@patch(
    "databricks.labs.doc_qa.evaluators.templated_evaluator.openai_provider.request_openai"
)
def test_openaievaluator(mock_request_openai):
    # Prepare the mock response
    mock_request_openai.return_value = {
        "function_call": {"arguments": json.dumps({"result": 5.0})}
    }
    ...
    openai_evaluator = OpenAIEvaluator(
        ...
        openai_function={
            "name": "addition",
            "parameters": {"properties": {"result": {"type": "number"}}},
        },
    )
...
@patch(
    "databricks.labs.doc_qa.evaluators.templated_evaluator.anthropic_provider.request_anthropic"
)
...
def test_openai_evaluator(mock_request_openai):
    ...
    mock_response = {"function_call": {"arguments": '{"score": 3.5}'}}
    ...
    openai_evaluator = OpenAIEvaluator(
        ...
        openai_function={
            ...
            "parameters": {"properties": {"score": {"type": "number"}}},
        },
    )
...
def test_eval_result():
    rows = [
        RowEvalResult(is_successful=True, error_msg=None, score=3.5),
        RowEvalResult(is_successful=True, error_msg=None, score=4.5),
    ]
