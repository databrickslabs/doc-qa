...
def test_openai_embedding_provider_batch_query(mocker):
    mocker.patch('openai.Embedding.create', return_value={'data': [{'embedding': [0.1, 0.2, 0.3]}, {'embedding': [0.4, 0.5, 0.6]}]})
    provider = OpenAIEmbeddingProvider(api_key='test-key')
...
def test_build_faiss_index(mock_faiss_index, mock_document, mocker):
    ...
    retriever = CsvRetriever(embedding_provider=mocker.MagicMock(), documents=[mock_document], column_names=['title', 'content'])
    retriever._build_faiss_index()
    mock_faiss_index.assert_called_once_with(len(mock_document.vector))
    assert retriever.id_to_document == {0: mock_document}
...
def test_save_as_csv(mock_to_csv, mock_document):
    ...
    retriever = CsvRetriever(embedding_provider=MagicMock(), documents=[mock_document], column_names=['title', 'content'])
...
def test_load_from_csv(mock_read_csv, mock_embedding_provider, mock_document):
    mock_read_csv.return_value = pd.DataFrame.from_records([{'title': 'test title', 'content': 'test content', 'vector': json.dumps([0.1, 0.2, 0.3]), 'created_at': str(mock_document.created_at)}])
    retriever = CsvRetriever.load_from_csv('test_path.csv', embedding_provider=mock_embedding_provider)
    assert retriever.documents[0].vector == [0.1, 0.2, 0.3]
...
def test_index_from_csv(mock_read_csv, mock_embedding_provider, mocker):
    ...
    mock_read_csv.return_value = pd.DataFrame.from_records([{'title': 'test title', 'content': 'test content', 'vector': [0.1, 0.2, 0.3]}])
    template = mocker.MagicMock()
    retriever = CsvRetriever.index_from_csv('test_path.csv', embed_prompt_template=template, embedding_provider=mock_embedding_provider)
    assert retriever.documents[0].vector == mock_embedding_provider.embed_queries.return_value[0]